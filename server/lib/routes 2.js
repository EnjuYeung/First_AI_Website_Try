...
const uploadedIconFilename = (url) => {
  const match = /^\/api\/uploads\/([a-f0-9-]+\.(?:png|jpg|webp))$/i.exec(String(url || ''));
  return match?.[1] || '';
};
...
export const registerRoutes = ({
  ...
  app.post('/api/telegram/webhook/:token', async (req, res) => {
    ...
    try {
      ...
      let result = null;
      const updated = await storage.updateUserData(user, (current) => {
        const currentTelegram = current.settings?.notifications?.telegram || {};
        if (currentTelegram.botToken !== incomingToken) throw new Error('invalid_token');
        const templateStr =
          current.settings?.notifications?.rules?.template || DEFAULT_REMINDER_TEMPLATE_STRING;
        ...
        result = applySubscriptionAction(sub, action);
        if (action === 'renewed') {
        ...
        updateRenewalFeedback(current.notifications, sub, currentBillingDate, feedbackValue);
        return current;
      });
  ...
  app.patch('/api/data', auth.authMiddleware, async (req, res) => {
    ...
    const validationError = validateDataPatch(payload);
    if (validationError) {
      return res.status(400).json({ success: false, message: validationError });
    }
    let removedIconFilenames = [];
    const updated = await storage.updateUserData(req.user.username, (current) => {
      if (payload.subscriptions) {
        const nextIcons = new Set(
          payload.subscriptions.map((sub) => uploadedIconFilename(sub.iconUrl)).filter(Boolean)
        );
        removedIconFilenames = (current.subscriptions || [])
          .map((sub) => uploadedIconFilename(sub.iconUrl))
          .filter((filename) => filename && !nextIcons.has(filename));
      }
      return { ...current, ...payload };
    });
    await Promise.all(
      removedIconFilenames.map((filename) =>
    ...
    try {
      ...
      if (test) {
        ...
        const tested = await storage.updateUserData(username, (current) => {
          ...
          current.settings.exchangeRateApi.enabled = true;
          current.settings.exchangeRateApi.lastTestedAt = Date.now();
          return current;
        });
  ...
  app.post('/api/2fa/init', auth.authMiddleware, async (req, res) => {
    ...
    await storage.updateUserData(req.user.username, (current) => {
      current.settings.security = current.settings.security || {};
  ...
  app.post('/api/2fa/verify', auth.authMiddleware, async (req, res) => {
    ...
    await storage.updateUserData(req.user.username, (current) => {
      ...
      security.pendingTwoFactorSecret = '';
  ...
  app.post('/api/2fa/disable', auth.authMiddleware, async (req, res) => {
    await storage.updateUserData(req.user.username, (current) => {
      current.settings.security.twoFactorEnabled = false;
      current.settings.security.twoFactorSecret = '';
      current.settings.security.pendingTwoFactorSecret = '';
      return current;
    });
    res.json({ success: true });
  });
